...
The Python package lives at the plugin root. Run the module from there:
...
If `python -m dbmigrate` is not found, run it from the plugin directory, e.g.
`PYTHONPATH="${CLAUDE_PLUGIN_ROOT}" python -m dbmigrate ...` (or from the repo
root that contains the `dbmigrate/` package).
